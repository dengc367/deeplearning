...
def create_model(dnn_feature_columns,
    ...
    query_emb_list = embedding_lookup(embedding_dict,
                                      ...
                                      to_list=True)
    hist_emb_list = embedding_lookup(embedding_dict,
                                     features,
                                     history_feature_columns,
    ...
    for i in range(transformer_num):
        att_embedding_size = transformer_output.get_shape().as_list(
        )[-1] // att_head_num
        transformer_layer = Transformer(att_embedding_size=att_embedding_size,
        ...
        transformer_output = transformer_layer([
            transformer_output, transformer_output, user_behavior_length,
            user_behavior_length
        ])

    attn_output = AttentionSequencePoolingLayer(att_hidden_units=(64, 16),
...
class BST(BaseModel):
    def __init__(self, dnn_feature_columns, history_feature_list, **kwargs) -> None:
        model = create_model(dnn_feature_columns, history_feature_list, **kwargs)
